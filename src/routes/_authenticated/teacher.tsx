...
import { createCourse, updateCourse, updateInstructorProfile, deleteCourse } from "@/lib/courses.functions";
...
function TeacherWorkspacePage() {
  ...
  const togglePauseMutation = useMutation({
    mutationFn: ({ courseId, currentStatus }: { courseId: string; currentStatus: string }) => {
      const nextStatus = currentStatus === "published" ? "paused" : "published";
      return updateCourse({ data: { courseId, status: nextStatus } });
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["instructorCourses"] });
      const verb = variables.currentStatus === "published" ? "paused" : "published";
      toast.success(`Course is now ${verb}!`);
    },
    onError: (err: Error) => {
              ...
              return (
                <div key={course.id} className="rounded-2xl border border-border bg-surface p-6 space-y-4 shadow-sm flex flex-col justify-between">
                  ...
                  <div className="pt-3 border-t border-border flex items-center justify-between gap-2 flex-wrap">
                    <span className={`text-xs font-semibold capitalize px-2.5 py-0.5 rounded-full border ${
                    ...
                    {isOwner ? (
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => togglePauseMutation.mutate({ courseId: course.id, currentStatus: course.status })}
                          title={course.status === "published" ? "Pause Course" : "Publish Course"}
                          className="inline-flex items-center gap-1 rounded-lg border border-border bg-surface hover:bg-surface-strong px-2.5 py-1 text-xs font-medium text-ink transition"
                        >
                          {course.status === "published" ? (
                            <>
                        ...
                        <button
                          ...
                          title="Edit Course & Materials"
                          className="inline-flex items-center gap-1 rounded-lg bg-ink text-background px-3 py-1 text-xs font-medium hover:bg-ink/90 transition shadow-sm"
                        >
                        ...
                        <button
                          ...
                          title="Delete Course"
                          className="inline-flex items-center justify-center rounded-lg border border-rose-200 dark:border-rose-900 bg-rose-50 dark:bg-rose-950/40 p-1.5 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/60 transition"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="w-full max-w-xl rounded-2xl border border-border bg-surface p-6 md:p-8 shadow-xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-ink-muted mb-1 font-medium">Course Description</label>
                  <textarea
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    rows={2}
                    className="w-full rounded-xl border border-border bg-background p-3 text-xs text-ink focus:outline-none focus:border-ink font-medium"
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-ink-muted mb-1 font-medium">Course Visibility / Status</label>
                  <select
                    value={courseStatus}
                    onChange={(e) => setCourseStatus(e.target.value)}
                    className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-xs text-ink focus:outline-none focus:border-ink font-medium"
                  >
                    <option value="published">Published (Live in Student Catalog)</option>
                    <option value="paused">Paused (Hidden from Catalog)</option>
